...
use super::{common::QuadIndexBufferResources, Component, Drawable};
...
impl<'a> StoreResources<BackendContext<'a>> for TextResources {
    fn initialize(ctx: &BackendContext<'a>) -> Self {
        let shader = init_glyph_shader(ctx.device);
        let pipeline_layout =
            init_glyph_pipeline_layout(ctx.device, ctx.textures.bind_group_layout());
...
impl GlyphComponent {
    pub fn from_glyph(glyph: &Glyph, ctx: &mut DrawContext, depth: f32) -> Self {
        let coords = glyph.bounds.into_coords();
...
impl Component for GlyphComponent {
    ...
    fn render_transparent<'rpass>(
        ...
        let text_resources = ctx.get_resources::<TextResources>();

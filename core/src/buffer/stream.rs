...
use wgpu::{
    Buffer, BufferAddress, BufferBinding, BufferUsages, Device, ImageCopyBuffer, ImageDataLayout,
};

use crate::{buffer::GrowingBuffer, pass::RenderBufferSlice};
...
impl StreamBuffer<'_> {
    pub fn as_image_copy(
        ...
        bytes_per_row: u32,
        rows_per_image: u32,
    ) -> Option<ImageCopyBuffer> {
        if bytes_per_row as u64 * rows_per_image as u64 >= slice.size {
            None
        } else {
            Some(ImageCopyBuffer {
                buffer: &self.buffer,
...
pub fn write_test() {
    ...
    use crate::test::create_test_backend;
    ...
    allocator.flush(backend.device());
